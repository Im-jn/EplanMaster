...
def match_shapes(sniffer: vector_sniffer, hits: list[dict[str, object]], page_num: int, debug: bool = False) -> list[dict[str, object]]:
    sniffer.goto(page_num)
...
def main() -> None:
    ...
    with vector_sniffer(pdf_path) as sniffer:
        ...
        for page_num in range(1, sniffer.page_count + 1):
            matched_groups = match_shapes(sniffer, hits, page_num)
            print(json.dumps({"page_num": page_num, "matched_group_count": len(matched_groups)}, ensure_ascii=False, indent=2))
